...
void RingHashLoadBalancer::Ring::create(Runtime::Loader& runtime,
  ...
  DVLOG(2) << "ring hash: building ring";
  ring_.clear();
  if (hosts.empty()) {
    return;
  }
  ...
  DVLOG(2) << fmt::format("ring hash: min_ring_size={} hashes_per_host={}", min_ring_size,
                         hashes_per_host);
  ...
  for (const auto& host : hosts) {
    for (uint64_t i = 0; i < hashes_per_host; i++) {
      ...
      DVLOG(2) << fmt::format("ring hash: hash_key={} hash={}", hash_key, hash);
  ...
  for (auto entry : ring_) {
    DVLOG(2) << fmt::format("ring hash: host={} hash={}", entry.host_->address()->asString(),
                           entry.hash_);
  }
#endif
}
