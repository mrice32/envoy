...
CodecClient::CodecClient(Type type, Network::ClientConnectionPtr&& connection,
  ...
  LOG(INFO) << format_connection_log("connecting", *connection_);
  connection_->connect();
...
void CodecClient::onData(Buffer::Instance& data) {
  ...
  try {
    ...
    LOG(INFO) << format_connection_log("protocol error: {}", *connection_, e.what());
    close();
    protocol_error = true;
  } catch (PrematureResponseException& e) {
    LOG(INFO) << format_connection_log("premature response", *connection_);
    close();
