...
void ConnPoolImpl::checkForDrained() {
  ...
  if (drained) {
    DVLOG(1) << "invoking drained callbacks";
...
ConnectionPool::Cancellable* ConnPoolImpl::newStream(Http::StreamDecoder& response_decoder,
  ...
  if (!host_->cluster().resourceManager(priority_).requests().canCreate()) {
    DVLOG(1) << "max requests overflow";
    callbacks.onPoolFailure(ConnectionPool::PoolFailureReason::Overflow, nullptr);
    host_->cluster().stats().upstream_rq_pending_overflow_.inc();
  } else {
    DVLOG(1) << format_connection_log("creating stream", *primary_client_->client_);
...
void ConnPoolImpl::onConnectionEvent(ActiveClient& client, uint32_t events) {
  if ((events & Network::ConnectionEvent::RemoteClose) ||
    ...
    if (&client == primary_client_.get()) {
      DVLOG(1) << format_connection_log("destroying primary client", *client.client_);
      dispatcher_.deferredDelete(std::move(primary_client_));
    } else {
      DVLOG(1) << format_connection_log("destroying draining client", *client.client_);
...
void ConnPoolImpl::movePrimaryClientToDraining() {
  DVLOG(1) << format_connection_log("moving primary to draining", *primary_client_->client_);
  if (draining_client_) {
...
void ConnPoolImpl::onConnectTimeout(ActiveClient& client) {
  DVLOG(1) << format_connection_log("connect timeout", *client.client_);
  host_->cluster().stats().upstream_cx_connect_timeout_.inc();
  client.client_->close();
}

void ConnPoolImpl::onGoAway(ActiveClient& client) {
  DVLOG(1) << format_connection_log("remote goaway", *client.client_);
  if (&client == primary_client_.get()) {
    movePrimaryClientToDraining();
  }
}
...
void ConnPoolImpl::onStreamDestroy(ActiveClient& client) {
  DVLOG(1) << format_connection_log("destroying stream: {} remaining", *client.client_,
