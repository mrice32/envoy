...
void WatcherImpl::addWatch(const std::string& path, uint32_t events, OnChangedCb callback) {
  ...
  DVLOG(1) << fmt::format("added watch for directory: '{}' file: '{}' fd: {}", directory, file,
                         watch_fd);
...
void WatcherImpl::onInotifyEvent() {
  while (true) {
    ...
    while (index < rc) {
      ...
      DVLOG(1) << fmt::format("notification: fd: {} mask: {:x} file: {}", file_event->wd,
      ...
      for (FileWatch& watch : callback_map_[file_event->wd].watches_) {
        if (watch.file_ == file && (watch.events_ & events)) {
          DVLOG(1) << fmt::format("matched callback: file: {}", file);
          watch.cb_(events);
        }
      }
